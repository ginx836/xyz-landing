import { useState } from "react";
//motion
import { motion } from "framer-motion";
//variants
import { fadeIn } from "../variants";

const Pricing = () => {
  const [isYearly, setIsYearly] = useState(false);

  const packages = [
    {
      name: "Starter",
      monthlyPrice: 19,
      yearlyPrice: 199,
      description: "Descriptive text about programme",
      green: "/src/assets/greenCheck.png",
    },
    {
      name: "Advance",
      monthlyPrice: 39,
      yearlyPrice: 399,
      description: "Descriptive text about programme",
      green: "/src/assets/greenCheck.png",
    },
    {
      name: "Premium",
      monthlyPrice: 59,
      yearlyPrice: 599,
      description: "Descriptive text about programme",
      green: "/src/assets/greenCheck.png",
    },
  ];

  return (
    <div className="md:px-14 p-4 max-w-screen-2xl mx-auto py-10" id="pricing">
      <div className="text-center">
        <h2 className="md:text-5xl text-3xl font-extrabold text-primary mb-2">
          Nos différents programmes
        </h2>
        <p className="text-tertiary ld:w-1/3 mx-auto px-4">
          Lorem ipsum, dolor sit amet consectetur adipisicing elit. Adipisci
          expedita minus voluptas soluta iste laboriosam, aliquam facere maxime
          vitae magni eos.
        </p>

        {/* toggle pricing */}
        <div className="mt-16">
          <label
            htmlFor="toggle"
            className="inline-flex items-center cursor-pointer"
          >
            <span className="mr-8 text-2xl font-semibold">Mensuel</span>
            <div className="w-14 h-6 bg-gray-300 rounded-full transition duration-300 ease-in-out">
              <div
                className={`w-6 h-6 rounded-full transition duration-200 ease-in-out ${
                  isYearly ? "bg-primary ml-8" : "bg-gray-500"
                }`}
              ></div>
            </div>
            <span className="ml-8 text-2xl font-semibold">Annuel</span>
          </label>
          <input
            type="checkbox"
            id="toggle"
            className="hidden"
            checked={isYearly}
            onChange={() => setIsYearly(!isYearly)}
          />
        </div>
      </div>
      {/* princing card */}
      <motion.div
        variants={fadeIn("up", 0.2)}
        initial="hidden"
        whileInView={"show"}
        viewport={{ once: false, amount: 0.5 }}
        className="grid sm:gid-cols-2 lg:grid-cols-3 gap-10 mt-20 md:w-11/12 mx-auto"
      >
        {packages.map((pkg, index) => (
          <div
            key={index}
            className="border py-10 md:px-6 px-4 rounded-lg shadow-3xl"
          >
            <h3 className="text-3xl font-bold text-center text-primary">
              {pkg.name}
            </h3>
            <p className="text-tertiary text-center my-5">{pkg.description}</p>
            <p className="mt-5 text-center text-secondary text-4xl font-bold ">
              {isYearly ? `$${pkg.yearlyPrice}` : `$${pkg.monthlyPrice}`}{" "}
              <span className="text-base text-tertiary font-medium">
                / {isYearly ? "an" : "mois"}
              </span>
            </p>
            <ul className="mt-4 space-y-2 px-4">
              <li className="flex gap-3 items-center">
                <img src={pkg.green} alt="Pastille verte" className="w-4 h-4" />{" "}
                Accès aux vidéos des cours
              </li>
              <li className="flex gap-3 items-center">
                <img src={pkg.green} alt="Pastille verte" className="w-4 h-4" />{" "}
                Exercices corrigés
              </li>
              <li className="flex gap-3 items-center">
                <img src={pkg.green} alt="Pastille verte" className="w-4 h-4" />{" "}
                Cours additionnels
              </li>
              <li className="flex gap-3 items-center">
                <img src={pkg.green} alt="Pastille verte" className="w-4 h-4" />{" "}
                Conférences mensuelles
              </li>
              <li className="flex gap-3 items-center">
                <img src={pkg.green} alt="Pastille verte" className="w-4 h-4" />{" "}
                Conseils personnalisés
              </li>
            </ul>
            <div className="w-full mx-auto mt-8 flex items-center justify-center">
              <button className="btnPrimary">Démarrer</button>
            </div>
          </div>
        ))}
      </motion.div>
    </div>
  );
};

export default Pricing;
